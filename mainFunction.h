...
int pipefds[100][9][2];
int pipes[100][2];
int pipescore[100][100][2];
int pipeitem[100][100][2];
     ...
     /*   double scoreuser;
       if(data->store+1==atoi(check)){
           ...
           for(int h=1;h<=data->n;h++){
               read(pipeitem[data->user][h][0], buffer1, sizeof(buffer1));  
               write(pipeitem[data->user][h][1], buffer1, sizeof(buffer1));
               if(strcmp(buffer1,data->itemName)==0){
                 read(pipeitem[data->user][h][0], buffer2, sizeof(buffer2));  
                 printf("%s \n",buffer2);
...
void main_function(int user, char **items, int *numitems, int n, int treshhold, int is_repeated, char* user_name){
//inputs
    // int user=0;
    // int treshhold=577;
    // char items[2][100] ;
    ...
    int numitems[2]={2,2};
    int n=2; int treshhold=50000; int is_repeated=0;*/
...
write(pipes[user][1], st, 2); 
 ...
 for (int i = 1; i <= n; i++)
    {
        pipe(pipescore[user][i]);
        pipe(pipeitem[user][i]);
        write(pipeitem[user][i][1],items[i-1],10);
        char itemnamescor[50];
        sprintf(itemnamescor,"%.1f",user_scores_2_goods[i]);
         
        printf("Score for good %d: %.1f\n", i, user_scores_2_goods[i]);
        
        write(pipescore[user][i][1], itemnamescor ,10);
