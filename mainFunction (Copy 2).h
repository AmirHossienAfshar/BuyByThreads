...
void *thread_function(void *arg)
{
    ...
    if (res.foundItem == 1)
    {
        ...
        if (data->store + 1 == atoi(check))
        {
            ...
            printf("\n  input1:%d \n ", data->input1);
            read(pipescore[data->user][data->input1][0], buffer2, sizeof(buffer2));
            printf("\n   score:%s \n", buffer2);
...
//     }
    ...
    if (scores[maxId] > -1 && user_scores_2_goods[0])
    {
        ...
        for (int i = 1; i <= n; i++)
        {
            ...
            sprintf(itemnamescor, "%d", user_scores_2_goods[i]);
            // strcpy(itemnamescor, "%.1f", user_scores_2_goods[i]);
            printf("Score for good %d: %s\n", i, itemnamescor);
            write(pipescore[user][mypathes257[i - 1]][1], itemnamescor, sizeof(itemnamescor));
